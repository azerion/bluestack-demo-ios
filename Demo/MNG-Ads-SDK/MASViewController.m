...
- (IBAction)createHimono:(UIButton *)sender {
    ...
    bannerAdsFactory.placementId = MNG_MAS_BANNER_PLACEMENT_ID;
    MNGAdSize size;
...
- (IBAction)createSashimi:(UIButton *)sender {
    ...
    bannerAdsFactory.clickDelegate = self;
    bannerAdsFactory.viewController = [APP_DELEGATE window].rootViewController;
    bannerAdsFactory.placementId = MNG_MAS_SQUARE_BANNER_PLACEMENT_ID;
    MNGAdSize size;
...
- (IBAction)createNativeAf:(UIButton *)sender {
    ...
    nativeAdsFactory.placementId = MNG_ADS_NATIVE_MAS_PLACEMENT_ID;
    MNGPreference *preferences = [Utils getTestPreferences];
...
- (IBAction)createInterAf:(UIButton *)sender {
    ...
    interstitialAdsFactory.placementId = MNG_ADS_INTERMAS_PLACEMENT_ID;
    /**
...
-(void)adsAdapter:(MNGAdsAdapter *)adsAdapter bannerDidLoad:(UIView *)adView preferredHeight:(CGFloat)preferredHeight{
    ...
    adView.frame = CGRectMake((self.view.frame.size.width-width)/2, self.view.frame.size.height - preferredHeight -40, width, preferredHeight);
